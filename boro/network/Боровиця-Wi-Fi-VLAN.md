...
|---|---|---|---|---|
| CC:2D:E0:ED:18:56 | Capelin (заміна cAP#1) | 2.4 | cfg-fubar-2g-cap1 | **ch6** (2437) |
| CC:2D:E0:ED:18:57 | Capelin | 5 | cfg-fubar-5g-cap1 | **ch44** (80 МГц, блок 36–48, eeCe) |
...
**Статус: cAP#1 списано (несправне залізо). Замінено на Capelin (б/у cAP ac) — розгорнуто 2026-06-18, у CAPsMAN `Run`, ch6/ch44. Мережу тримають Capybara (cAP#2) + Capelin. wAP (Wapiti) наразі знятий — його місце на ether2 Capybara зайняла Capelin. Wi-Fi працює.**
...
**Схема імен (непозиційна, уся жива природа):** оскільки CAPsMAN провіжить за radio-MAC, а не за позицією, пристроям дано текстові імена замість порядкових — щоб ставити в будь-якому порядку без плутанини:
- **Hapuku** = hAP (роутер-контролер; новозеландський морський окунь 🐟)
- **Capybara** = cAP#2 (`CC:2D:E0:ED:1A:38`)
- **Capelin** = заміна cAP#1 (мойва; пристрій `CC:2D:E0:ED:18:54`, radio-MAC 2.4=`:18:56` / 5=`:18:57`, S/N 81CE0833CA25)
- **Wapiti** = wAP (`CC:2D:E0:85:BE:6F`; вапіті/елк)

(Перейменування identity — косметика, CAP-з'єднання не рве. Capelin вже має identity `Capelin`; Hapuku/Capybara/Wapiti — перейменувати при нагоді: `/system identity set name=…`.)
...
**Розгортання Capelin (2026-06-18):**
- Стрес на столі: пережила кілька холодних стартів + соак без зависань (на відміну від мерця-cAP#1). Узято в прод.
- Підключена в **ether2 Capybara** (той самий trunk-порт VLAN 10/20/50). wAP при цьому знятий.
- Конфіг залито зі **свіжого дефолту через WebFig** (одним рядком через `;`, бо WebFig збиває багаторядкову вставку). Builtin-бридж `bridge` **не чіпати** (`set name=bridge` падає `cannot remove builtin`) — використано як є: додано `ether1` у бридж trunk'ом (tagged 10/20/50), піднято `vlan10-mgmt` + IP `10.20.10.11`, прибрано дефолтні dhcp/`192.168.88.x`, увімкнено `wireless cap` на `10.20.10.1`. Послідовність: спершу mgmt-IP, аж потім CAP-режим — щоб при обриві точка лишилась керованою.
- **Нові radio-MAC** (інші, ніж у мерця): 2.4=`CC:2D:E0:ED:18:56`, 5=`CC:2D:E0:ED:18:57`. На контролері додано provisioning-правила під них (→ `cfg-fubar-2g-cap1`/`cfg-fubar-5g-cap1` = ch6/ch44); **видалено застарілі** правила під мертві `:18:46`/`:18:47`.
- Перепровіжн прибрав тимчасові статичні маски (дефолтні ch7/ch157 @30dBm) → Capelin стала на **ch6 (2437) / ch44 (5220), 20 dBm**. Join `Run`, `.11` пінгується. Адмін-пароль задано.

**Незакрите:**
- Перейменувати identity живих: **Hapuku** (hAP), **Capybara** (cAP#2), **Wapiti** (wAP) — косметика.
- Доля **wAP (Wapiti)**: повернути в мережу (за Capelin/Capybara через ether2-trunk, або на hAP ether3) + дати канали ch11/ch48; зараз знятий.
- Стежити за рештою (б/у залізо) + тримати запасну точку як норму.
