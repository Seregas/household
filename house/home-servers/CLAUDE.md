...
### ⚠️ ВИПРАВЛЕННЯ (2026-08-02): у AM5D4ID2 Є ВИДІЛЕНИЙ IPMI-ПОРТ

**Раніше в цьому файлі записано хибно** (моя помилкова інтерпретація): «виділеного IPMI-порту немає,
BMC через NCSI на LAN1». **Це НЕ так.** Специфікація ASRock згадує лише «2×LAN + IPMI», не виділяючи
IPMI окремим рядком — звідси хибний висновок. Насправді на панелі **три RJ45**: 2× i210 (ОС) +
**окремий IPMI-порт**.

**Що змінилось:**
- **BMC → виділений IPMI-порт → ether6** (VLAN 10, `10.10.10.5`). Власна фізична лінія,
  **незалежна від мережевих карт ноди**.
- **Обидва i210 тепер ВІЛЬНІ** — ОС уся на 10G через CRS305.
- Проблема «нода не бачить BMC» (кадр не вертається в порт, з якого вийшов) **зникла в корені** —
  вона була наслідком shared-порту. Тепер `ping 10.10.10.5` з ноди працює природно.
- `Keep Share NIC Link Up` більше не критичне (це налаштування для NCSI-режиму).

**Симптом, що привів до відкриття:** при вимкненій ноді BMC перестав відповідати (ARP incomplete,
100% loss навіть із роутера в тому ж VLAN), хоч ether6 показував 1Gbps link-ok. `ipmitool` неможливий
(потребує запущеної ОС) → замкнене коло: BMC у мережі немає, ОС не працює, дистанційно ввімкнути нічим.
**Рішення:** переткнути кабель у виділений IPMI-порт → BMC одразу відповів (0.5 ms, 0% loss),
вебморда й Power Control доступні.

**DNS (додано 2026-08-02):**
- `bmc.mgmt.home.arpa` A → 10.10.10.5; `bmc.home.arpa` CNAME → bmc.mgmt. ⚠️ BMC **тільки HTTPS**.
- `pve.mgmt.home.arpa` A → 10.10.10.10 (менеджмент VLAN 10); `pve.srv.home.arpa` → 10.10.30.10 (робоча).
- **Навіщо `<хост>.<vlan>`:** роутер має 5 адрес (по одній на VLAN) — без сегмента не розрізнити.
  Нода має 2. Короткий `<хост>.home.arpa` — CNAME на головну.

### Split DNS у Tailscale — локальні імена працюють ЗВІДУСІЛЬ (2026-08-02)

**Проблема:** з офісу працювали `pve.mining-owl.ts.net:8006` (MagicDNS) і `10.10.30.10:8006` (IP через
subnet-router), але **НЕ** `pve.srv.home.arpa` — бо записи `.home.arpa` живуть на роутері, а поза домом
DNS — офісний резолвер або MagicDNS (100.100.100.100), які про цей домен не знають.

**Рішення:** `login.tailscale.com` → **DNS → Nameservers → Add → Restrict to domain**:
домен `home.arpa`, сервер **`10.10.30.1`** (роутер). Тепер усі пристрої в tailnet резолвлять локальні
імена — включно з `bmc.home.arpa`, `nas.srv.home.arpa`, `pbs.srv.home.arpa`. ✅ Перевірено з офісу.

⚠️ Чому саме `10.10.30.1` (роутер), а НЕ `10.10.30.15` (Pi-hole): Pi-hole живе в CT на ноді —
при вимкненій ноді локальні імена перестали б резолвитись і поза домом. Роутер тримає самі записи й працює завжди.

**Про безпеку доступу з офісу до приватних адрес — це НОРМА, не діра:** трафік іде зашифрованим
тунелем Tailscale (нода як subnet-router рекламує 10.10.10.0/24 + 10.10.30.0/24, ACL дозволяє
`group:admins`). Портів на роутері не проброшено — ззовні без Tailscale і ключа адреси недосяжні.

**Слабина, що ЛИШАЄТЬСЯ (TODO, тепер актуальніше):** subnet-router — це **сама нода**. Якщо вона
не завантажується, маршрут до VLAN 10 зникає разом із нею — саме тоді, коли BMC найпотрібніший.
BMC уже незалежний (власний порт), лишилось закрити другу половину:
- **Tailscale/WireGuard на RB5009** — роутер завжди живий і бачить BMC напряму (найнадійніше)
- або другий subnet-router у VLAN 10 (Mac mini `home-srv`)
- плюс **розумна розетка** на живлення ноди (Shelly/Tuya з планованого PDU) — ввімкнення
  незалежно від BMC узагалі
