...
def plan_geometry():
    ...
    # 29.07 (правило 1): втоплення 0.05 від стінок отворів ПРИБРАНО —
    # воно тоншало верх ребра сота↔кишеня до 1.45 (замір по STL).
    # Ціна: там, де фестонна межа fp біжить точно по стороні соти і
    # проходить через вершину гекса, лишаються пінч-вертикалі Z2..3
    # (нон-маніфолд кратності 4, «пісочний годинник» у плані) — об'єм
    # і bodies=1 правильні, прецедент прийняття: 16 ребер лінії
    # оскуляції стінок. Спроба лікувати перекриттям 0.3 у діри сот
    # ПОГІРШИЛА (33 замість 23: union не рухає вершину клину, а краї
    # buffer-язиків додали нових пінчів) — відкочено.
