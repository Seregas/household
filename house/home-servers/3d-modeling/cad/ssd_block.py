...
def build():
    ...
    with BuildPart() as blk:
        ...
        # язики-КЛИНИ, спрямовані НАЗАД (09.07 в4.3, фідбек: «вусики
        # назад, щоб защіпка не витягувала»): якір-стовпчик СПЕРЕДУ
        # скоби (y0.2..1.6, повна висота до бази через місток над
        # дахом), лезо йде від якоря НАЗАД під скобу і вливається в
        # полоз; клин 5.65→4.8 (пік біля якоря): при зсуві назад дах
        # наїжджає на клин — ДОТИСКАЄ блок до дна (натяг ~0.15)
        for xc in P.SNAP_TAB_XC:
            x0t = xc - P.SNAP_TAB_W / 2
            with BuildSketch(Plane.YZ.offset(x0t)) as tw:
                with BuildLine():
                    Polyline((0.2, P.SNAP_TAB_Z[0]),      # лезо+клин
                             (0.2, 5.65),
                             (by0f + 0.1, 4.8),
            ...
            with Locations((xc, 0.9, P.SNAP_TAB_Z[0])):    # якір
                Box(P.SNAP_TAB_W, 1.4, 8.6 - P.SNAP_TAB_Z[0], align=AMIN)
            # місток якір→база НАД дахом скоби (7.4; просвіт 0.2)
            with Locations((xc, (0.2 + by0f + 0.1) / 2, 7.6)):
                Box(P.SNAP_TAB_W, by0f + 0.1 - 0.2, 1.0, align=AMIN)
