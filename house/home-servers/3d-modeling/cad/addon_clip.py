"""
addon_clip.py — АДДОН-КЛІП (17.07 в3; історія: 09.07 «lsi_clip»
виделка-защіпка LSI, перейменовано — головна роль тепер тримати
ПАНЕЛЬ-АДДОН). Опускається ЗГОРИ ПІСЛЯ установки аддона:
  1. тримає АДДОН від вдавлення — БАЛКА від тіла вправо у НАСКРІЗНОМУ
     розрізі брови (17.07 «повний розріз»: брови у прольоті нема, балка
     на повну глибину відновлює хорду П-рами) опускається ЗА рамку
     аддона (губа t1.4, зазор 0.05, контакт 80.1..80.75); Y-стоп — ДВА
     ЗУБИ t1.4 за розрізами у надрізах ОБОХ брів (22.07: лівий зуб
     замість пронга з барбом — «кліп провалювався при вставці зліва»),
     crush-ребра тилів зубів (натяг 0.1 у стінки −94.8) вибирають люфт
     і додають тертя-ретенцію; балка і зуби СІДАЮТЬ низом на дно
     надрізів/паза 81.5 = жорсткий стоп посадки вниз (22.07: було —
     висіли 0.2, тіло пірнало в колодязь);
  2. у НАШОМУ варіанті заразом ПРИТРИМУЄ ПЛАТУ LSI — виделка в
     колодязі крізь брову: паз (лійка знизу) захоплює передній край
     карти.

Конструкція: тіло ±3.05 × y 4.9 (тил урівень з балкою −91.45; 17.07
ч.3 — історичний зріз −92.45 потовщено) × Z73.25..88.75 (верх урівень
панелі); низ = дві щоки 2.2 з пазом 1.7 (лійка-розтруб знизу) +
суцільна перемичка-упор 2мм спереду (край карти впирається по Y);
верх карти на LSI_BRK_TOP=80.75 накритий суцільним центром (карта
замкнена вертикально; 22.07: тіло суцільне — проріз пронга і барб
видалені разом із кишенею в панелі). Заднє верхнє ребро скруглене
R=BROW_R (повторює форму брови). Знімання — просто потягнути вгору:
crush-ребра — лише тертя (виїмка-підважка ВИДАЛЕНА 17.07 ч.3: полиця
дивилась угору, піднутрення нема).
Запуск: .venv/bin/python cad/addon_clip.py → out/addon_clip.step/stl
Координати збірки; друк — ЛИЦЕМ (y−96.35) вниз: тіло, балка і обидва
зуби копланарні спереду, лежать на столі.
"""
from build123d import *
import params as P
from exporter import save

AMIN = (Align.CENTER, Align.CENTER, Align.MIN)


def build():
    hw = P.LSI_SLOT_W / 2           # півширина паза 0.85
    bw = P.LSI_FORK_W               # щока 2.2
    xw = hw + bw                    # півширина тіла 3.05
    y0, y1 = -96.35, P.CAP_BEAM_YR  # тіло по Y НА ПОВНУ глибину (тил
    # −91.45 урівень з балкою; кліренс до стінки колодязя −91.0 = 0.45;
    # пронг глибшає 3.9→4.9 — жорсткість +25%, ε згину той самий)
    zb = P.LSI_FORK_Z[0] - 0.5      # низ щік 73.25
    zt = P.PANEL_H                  # верх урівень з панеллю
    z_card = P.LSI_BRK_TOP + 0.25   # стеля над картою 81.0
    with BuildPart() as clip:
        with Locations((P.LSI_X, (y0 + y1) / 2, zb)):
            Box(2 * xw, y1 - y0, zt - zb, align=AMIN)
        # паз карти: від перемички-упора (−94.3) назад НАСКРІЗЬ, до
        # стелі z_card; вище — суцільний центр (замок карти)
        with Locations((P.LSI_X, ((P.FRONT_Y + P.FRONT_PANEL_T + P.LSI_WEB_T + 0.1) + y1 + 1) / 2, zb - 0.5)):
            Box(2 * hw, y1 + 1 - (P.FRONT_Y + P.FRONT_PANEL_T + P.LSI_WEB_T + 0.1), z_card - (zb - 0.5),
                align=AMIN, mode=Mode.SUBTRACT)
        # лійка-розтруб знизу паза (самоцентрування на краю карти)
        for sx in (-1, 1):
            with BuildSketch(Plane.XZ.offset(94.3)) as fl:
                with BuildLine():
                    Polyline((P.LSI_X + sx * hw, zb + 2.0),
                             (P.LSI_X + sx * (hw + 0.8), zb - 0.01),
                             (P.LSI_X + sx * hw, zb - 0.01),
                             close=True)
                make_face()
            extrude(fl.sketch, amount=-(y1 + 1 - (P.FRONT_Y + P.FRONT_PANEL_T + P.LSI_WEB_T + 0.1)),
                    mode=Mode.SUBTRACT)
        # фаска знизу перемички-упора (з'їзд на край карти по Y)
        with BuildSketch(Plane.YZ.offset(P.LSI_X - hw)) as wch:
            with BuildLine():
                Polyline(((P.FRONT_Y + P.FRONT_PANEL_T + P.LSI_WEB_T + 0.1), zb + 1.2), ((P.FRONT_Y + P.FRONT_PANEL_T + P.LSI_WEB_T + 0.1), zb - 0.01),
                         (-95.5, zb - 0.01), close=True)
            make_face()
        extrude(wch.sketch, amount=2 * hw, mode=Mode.SUBTRACT)
        # (22.07: лівий проріз пронга 0.5 + барб 0.35 ВИДАЛЕНІ — «кліп
        # провалюється при вставці зліва»; пружину замінив ЛІВИЙ ЗУБ у
        # надрізі лівої брови, див. нижче; тіло тепер суцільне)
        # ── БАЛКА-ковпак (17.07 «повний розріз»; історія в3 «лезо у
        # пазу 1.6»): брова у прольоті розрізана НАСКРІЗЬ — балка на
        # ПОВНУ глибину брови (тил CAP_BEAM_YR=−91.45, врівень з тилом
        # брови −0.05) відновлює хорду П-рами і блокує ВДАВЛЕННЯ аддона
        # (губа за рамкою, зазор 0.05 лице↔тил рамки). Перекриття з
        # тілом 0.45 по X; зазори: 0.05 перед, 0.4 правий кінець
        # (розріз 133.4). 22.07: низ 81.7→81.5 — балка/зуби СІДАЮТЬ на
        # дно надрізів 81.5 = жорсткий стоп посадки вниз (було: висіла
        # 0.2, тіло пірнало в колодязь) ──
        yf = y0                                  # лице балки −96.35
        yr = yf + P.CAP_BLADE_T                  # тил губи/зуба −94.95
        bx0 = P.LSI_X + xw - 0.45                # 91.0
        with Locations(((bx0 + P.CAP_BLADE_X1) / 2, (yf + P.CAP_BEAM_YR) / 2,
                        (81.5 + zt) / 2)):
            Box(P.CAP_BLADE_X1 - bx0, P.CAP_BEAM_YR - yf, zt - 81.5)
        # нижня ГУБА t1.4 — контакт за рамкою 80.1..80.75 (вище рамка
        # фаскована ADP_CHAMF). 17.07 ч.3: gx0 = bx0 — губа зростається
        # з тілом (щілина 0.2, залишок в3-паза «лезо у пазу», заповнена;
        # x91..92.2 z79.7..82 — повітря колодязя, посадці не заважає)
        gx0 = bx0                                # 91.0
        with Locations(((gx0 + P.CAP_BLADE_X1) / 2, (yf + yr) / 2,
                        (P.CAP_BOT_Z + 82.0) / 2)):
            Box(P.CAP_BLADE_X1 - gx0, P.CAP_BLADE_T,
                82.0 - P.CAP_BOT_Z)
        # фаска 45° нижнього лицьового ребра губи (захід за рамку);
        # старт за правою гранню тіла +0.05 — не різати грань тіла
        gx_ch = P.LSI_X + xw + 0.05              # 91.5
        with BuildSketch(Plane.YZ.offset(gx_ch)) as bch:
            with BuildLine():
                Polyline((yf - 0.1, P.CAP_BOT_Z + 0.4),
                         (yf - 0.1, P.CAP_BOT_Z - 0.1),
                         (yf + 0.4, P.CAP_BOT_Z - 0.1), close=True)
            make_face()
        extrude(bch.sketch, amount=P.CAP_BLADE_X1 - gx_ch + 0.2,
                mode=Mode.SUBTRACT)
        # ── ЗУБ (Y-стоп вдавлення): наскрізний розріз задньої стінки
        # не має — зуб t1.4 продовжує передню смугу балки ЗА розріз
        # (133.4) у НАДРІЗ правої брови (до вежі 134.9); тил зуба
        # −94.95, стінка надрізу −94.8 → хід 0.15, вибирається
        # crush-ребром (виступ 0.25 → натяг 0.1); конус-лійка знизу
        # (кліп заходить згори) ──
        with Locations(((P.CAP_BLADE_X1 + P.CAP_TOOTH_X1) / 2,
                        (yf + yr) / 2, (81.5 + zt) / 2)):
            Box(P.CAP_TOOTH_X1 - P.CAP_BLADE_X1, P.CAP_BLADE_T,
                zt - 81.5)
        ry = yr - P.CAP_RIB_R + P.CAP_RIB_PROT       # центр −95.2
        with Locations((P.CAP_RIB_X, ry, P.CAP_RIB_Z[0])):
            Cylinder(P.CAP_RIB_R,
                     P.CAP_RIB_Z[1] - P.CAP_RIB_Z[0], align=AMIN)
        with Locations((P.CAP_RIB_X, ry, P.CAP_RIB_Z[0] - 0.4)):
            Cone(0.1, P.CAP_RIB_R, 0.4, align=AMIN)
        # ── ЛІВИЙ ЗУБ (22.07, дзеркально правому — замість пронга з
        # барбом): продовжує передню смугу тіла ВЛІВО (перекриття 0.45)
        # у НАДРІЗ лівої брови (стінка надрізу 83.4, кінець зуба 83.6 —
        # 0.2 зазору); те саме crush-ребро в задню стінку −94.8 ──
        tx1 = P.LSI_X - xw + 0.45                    # 85.8
        with Locations(((P.CAP_TOOTH_X0 + tx1) / 2,
                        (yf + yr) / 2, (81.5 + zt) / 2)):
            Box(tx1 - P.CAP_TOOTH_X0, P.CAP_BLADE_T,
                zt - 81.5)
        with Locations((P.CAP_RIB_XL, ry, P.CAP_RIB_Z[0])):
            Cylinder(P.CAP_RIB_R,
                     P.CAP_RIB_Z[1] - P.CAP_RIB_Z[0], align=AMIN)
        with Locations((P.CAP_RIB_XL, ry, P.CAP_RIB_Z[0] - 0.4)):
            Cone(0.1, P.CAP_RIB_R, 0.4, align=AMIN)
        # (17.07: ГРЕБІНКА-вирізи + хребет ВИДАЛЕНІ разом із
        # мостоопорами паза — брови над пазом більше нема)
        # ── закруглення тилу-верху R=BROW_R (фідбек «повторювало форму
        # брови»): один ланцюг x85.35..133.0 — тіло+балка, тили (−91.45)
        # і верхи (zt) копланарні (22.07: проріз пронга видалено — ребро
        # одним сегментом). Зуби (тил −94.95) не чіпаємо ──
        def _rear_top(e):
            bb = e.bounding_box()
            return (bb.size.X > 1 and bb.size.Z < 1e-6
                    and abs(bb.max.Z - zt) < 1e-6
                    and abs(bb.max.Y - P.CAP_BEAM_YR) < 1e-6)
        fillet(clip.edges().filter_by(_rear_top), radius=P.BROW_R)
        # (17.07 ч.3 фідбек #2: виїмка-підважка ВИДАЛЕНА — полиця дивилась
        # УГОРУ, піднутрення для нігтя не було → підняти не допомагала.
        # Кліп виходить простим тягненням угору: барб 45/45 симетричний,
        # crush-ребро зуба — лише тертя.)
    return clip.part


if __name__ == "__main__":
    part = build()
    print("valid:", part.is_valid, "| volume:", round(part.volume, 1))
    save(part, "addon_clip")
