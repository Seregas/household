...
скруглене R=BROW_R (повторює форму брови). Знімання — підважити за
виїмку на задньому верхньому краї (17.07 ч.3: ВІДКРИТА до тилу —
ніготь/викрутка заходить ззаду) АБО просто потягнути вгору: барб
45/45 симетричний, crush-ребро — лише тертя.
...
def build():
    ...
    with BuildPart() as clip:
        ...
        # виїмка-підважка на задньому верхньому краї — ВІДКРИТА до тилу
        # (17.07 ч.3, фідбек): полиця 1.2×1.2, підчепити нігтем/
        # викруткою ззаду; ріжеться ПІСЛЯ філета
        with Locations((P.LSI_X, y1 - 1.2, zt - 1.2)):
            Box(4.0, 2.4, 1.2 + 1,
