...
в6.3 — клин-дотиск):
  1) зачеп у слоті дна: перетин із floor = ЛИШЕ клин-переднатяг
     (~0.03 мм³/зачеп — верх клина 1.455 > низ мембрани 1.4 на кромці
     слота, ЗАДУМ в6.3; до в6.3 було 0);
  2) зачеп у Т-пазу блока: перетин = 0 (горбик дотичний позаду);
...
if __name__ == "__main__":
    ...
    print("── 1) зачеп ∩ floor (лише клин-переднатяг ~0.03 мм³) ──")
    for (cell, c) in zip(P.SNAP_SSD_CELLS, clips):
        print(f"  cell {cell}: {vol(fl, c):.3f} мм³")

    print("── 2) зачеп ∩ block (Т-паз; має бути 0, горбик дотичний) ──")
    for (cell, c) in zip(P.SNAP_SSD_CELLS, clips):
