...
def build():
    ...
    # 08.07: диски зсунуті вдаль (SSD_Y -14..86) → вікно від -13 до 87
    # (де диск реально притуляється до стінки, +1 запасу з боків)
    with BuildPart() as dcut:
        with Locations((133.7, (P.SSD_Y[0] + 1.0 + P.SSD_Y[1] + 1.0) / 2,
                        (5.0 + 70.0) / 2)):
            Box(2.4, (P.SSD_Y[1] + 1.0) - (P.SSD_Y[0] + 1.0), 70.0 - 5.0)
    ...
    # закінчується на ~78, далі ребро було гостре. Профіль = дуга R2
    # на всю ширину смуги: (132.9, 3.2) → (134.9, 5.2), як ków поруч.
    # (Перша спроба різала засипку на 134.9..136.9 — «утоплена
    # площина», відкочено.)
    with BuildPart() as lr:
        with BuildSketch(Plane.XZ.offset(-78.0)) as s:
            with Locations((133.9, 4.0)):
                Rectangle(2.0, 2.0)
            with Locations((134.9, 3.0)):
                Circle(2.0, mode=Mode.SUBTRACT)
        extrude(s.sketch, amount=-(86.6 - 78.0))
