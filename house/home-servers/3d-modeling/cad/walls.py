...
def build():
    ...
    with BuildPart() as dcut:
        with Locations((133.7, (-24.0 + 86.4) / 2, (5.2 + 70.0) / 2)):
            Box(2.4, 86.4 - (-24.0), 70.0 - 5.2)
    right = (right - dcut.part).fix()
    # 06.07 (п.5+): «продовжити заокруглення бортику знизу» — ков R2
    # уздовж внутрішнього кута плита↔дно вікна (верх плінтуса), профіль
    # втоплений на 0.3 в обидві грані (уроки ков-стрічок)
    with BuildSketch(Plane.YZ.offset(P.WALL_R_X - P.WALL_T + 0.3)) as pc:
        with BuildLine():
            Polyline((-24.0, 5.2 - 0.3), (-24.0, 5.2 + 2.0),
                     (-24.0 + 0.001, 5.2 + 2.0))
        # профіль ковa окремим контуром нижче
        pass
    with BuildPart() as plc:
        with BuildSketch(Plane((P.WALL_R_X - P.WALL_T + 0.3, -24.0, 5.2),
                               x_dir=(-1, 0, 0), z_dir=(0, 1, 0))) as cv:
            with BuildLine():
                Polyline((-0.3, -0.3), (2.0, -0.3), (2.0, 0.0))
                RadiusArc((2.0, 0.0), (0.0, 2.0), -2.0)
                Polyline((0.0, 2.0), (-0.3, 2.0), (-0.3, -0.3))
            make_face()
        extrude(cv.sketch, amount=86.4 - (-24.0))
    right = (right + plc.part).fix()
