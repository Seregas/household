...
# (31.07 Фаза 0: rear_ridge() — задній бортик 5×5 — ВИДАЛЕНО за
#  рішенням користувача «замість трампліну і самого бортику». Зад тепер
#  закривається лише задньою рамою дна; за потреби — закритий ізогрід.)
...
def build():
    ...
    # (31.07 Фаза 0: ЗАДНІЙ БОРТИК + трамплін + зниження бортика
    # ВИДАЛЕНІ — рішення користувача «замість трампліну і самого
    # бортику». Зад = чиста задня рама дна; жорсткість заду додамо
    # ЗАКРИТИМ ізогрідом у товщі площини (Фаза 4) за потреби.
    # rear_ridge() знято; мертві P.RIDGE_TOP_Z/RIDGE_BOARD_Z/
    # RIDGE_LOW_X1/BEAD_H — на зачистку у Фазі 5.)
