...
Тримають (корпус → аддон → ковпак addon_clip.py):
  ...
  • ВЕРХ: КОВПАК-ЛЕЗО опускається в паз брови ЗА рамку (зазор 0.05) —
    тримає від вдавлення; підйом блокує ободок (зазор 0.15) + crush.
...
def _plans(variant):
    ...
    # ламель: КРИЛА half-lap зупиняються ПІД фаскою рамки (клин фаски
    # лишав би з крила 1.45 пір'яний сліверс), ядро прольоту — до верху
    lam = unary_union([
        sg.box(lx0, P.ADP_STRIP_Z, lx1, P.ADP_TOP_Z - P.ADP_CHAMF),
        sg.box(px0, P.ADP_STRIP_Z, px1, P.ADP_TOP_Z)])
...
def build(variant):
    ...
    with BuildPart() as ad:
        ...
        # скіс передньої грані язиків знизу (в3, качання): при нахилі
        # >3° передньо-нижнє ребро з'їдало зазор 0.2 до тилу смуги-
        # сідла (зонд: врізання 0.20 на z1.2 при 6°) → грань відкинута
        # назад до 0.26 на низу (кут скосу ~9.4° > 6°+запас); crush-
        # ребер (лице −95.8) не торкається
        yF = P.BODY_FRONT_Y + 0.2                        # −96.2
        wx0 = P.ADP_TON_XC[0] - P.ADP_TON_W / 2 - 0.5
        wx1 = P.ADP_TON_XC[-1] + P.ADP_TON_W / 2 + 0.5
        with BuildSketch(Plane.YZ.offset(wx0)) as tch:
            with BuildLine():
                Polyline((yF - 0.1, 3.4), (yF - 0.1, 1.1),
                         (yF + 0.28, 1.1), close=True)
            make_face()
