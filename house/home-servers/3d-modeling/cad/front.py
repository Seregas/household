...
def build():
    ...
    with BuildPart() as fp:
        ...
        # згори: НІШІ клин-бампів пружних пальців (стеля 52.9, дно 51.0 над
        # апертурою 50.1; задня стінка −96.9 = +Y стоп ПРОВАЛЮВАННЯ, передня
        # −98.35 лишає лице 1.05 — «ніша на фронтальній панелі», дискретна;
        # клин ловить бамп ізсередини, стеля=+Y стоп, передня стінка=−Y стоп)
        for xd in P.INS_DIMPLE_XC:
            with Locations((xd, (-98.35 + -96.9) / 2,
                            (51.0 + 52.9) / 2)):
