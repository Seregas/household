...
def plan_panel():
    ...
    # 23.07 «потужніші зачепи»: над кишенями бампів (стеля 52.9) —
    # локальні суцільні зони до 53.9 (ромбілі наскрізні; без цього
    # стінка стеля-кишені↔ромб була б 0.4 — а її бамп тисне вгору)
    port_pads = port_pads.union(unary_union(
        [sg.box(xd - P.INS_LIP_TAB_W / 2 - 1.5, P.IO_Z[1],
                xd + P.INS_LIP_TAB_W / 2 + 1.5, 53.9)
...
def build():
    ...
    with BuildPart() as fp:
        ...
        # знизу вгору; 23.07 «потужніші зачепи»: клин R1.2 з центром
        # −96.95 — перед клину −98.15 → передня стінка −98.35 (лице
        # лишає 1.05), поріг −96.9 (до тилу панелі 0.5), стеля 52.9
        # (бамп-верх 52.7 + 0.2); зачеплення за стелю фальца 51.7 = 1.0;
        # низ 51.0 — база клину спускається до 51.1 і при R1.2 стартує
        # ПЕРЕД дном фальца −97.9 (кишеня покриває хвіст, зазор 0.1)
        for xc in P.INS_DIMPLE_XC:
            with Locations((xc, (-98.35 + -96.9) / 2,
                            (51.0 + 52.9) / 2)):
                Box(P.INS_LIP_TAB_W + 1.0, -96.9 - -98.35, 52.9 - 51.0,
                    mode=Mode.SUBTRACT)
        # ── ГУБИ ФАЛЬЦА (22.07, «панель провалюється всередину»): 2
        # смужки-навіси над низом фальца між язичками; низ = дно контуру
        # фальца 3.7 (анкер у суцільну панель знизу), тил = тил панелі.
        # Вставка заходить КАЧАННЯМ: низ фланця (потоншений до «губки»
        # 0.65 навпроти губ) пірнає ЗА губи, верх докачується, пальці
        # клацають. Тиск усередину знизу тримає ГЕОМЕТРІЯ. Друк (лицем
        # вниз): губа = консоль першого шару 1.15 з якорем по краю z3.7
        # (прецедент — консолі 1.5-1.8 надрізів зуба) ──
        for xc in P.INS_LIP_XC:
            with Locations((xc, (-96.4 + -96.4 - P.INS_LIP_T) / 2,
                            (P.INS_LIP_Z[0] + P.INS_LIP_Z[1]) / 2)):
                Box(P.INS_LIP_W, P.INS_LIP_T,
                    P.INS_LIP_Z[1] - P.INS_LIP_Z[0])
