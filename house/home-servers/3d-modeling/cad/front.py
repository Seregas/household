...
def plan_panel():
    ...
    # 24.07 ч.2: окремих keepout-боксів над нішами більше не треба —
    # port_pads (буфер 3.2 → Z до 53.3) лишає над тиловими нішами
    # (верх 51.65) 1.65 і над лицьовими (верх 51.45) 1.85 суцільного
...
def build():
    ...
    with BuildPart() as fp:
        ...
        # ── ДИСКРЕТНІ кишені кріплень вставки (24.07 РЕДИЗАЙН після
        # примірки щитка): суцільний фальц ВИДАЛЕНО — його полиця з'їдала
        # глибину стінки апертури там, де пупирки сталевого щитка мали
        # тиснути (щиток не тримався). Тепер стінка апертури ПОВНА 3.0
        # скрізь, лише 4 локальні кишені (2 знизу, 2 згори) між пупирками. ──
        # знизу: ЗАХОПНІ кишені (закриті спереду −97.9 і ззаду −96.3 →
        # жорсткий язик тримає в ОБИДВА боки Y), Y −96.3…−97.9
        for xc in P.INS_TAB_XC:
            with Locations((xc, (-96.3 + -97.9) / 2,
                            P.IO_Z[0] - (1.4 + P.INS_TAB_H + 0.4) / 2)):
                Box(P.INS_TAB_W + 0.6, -96.3 - -97.9,
                    1.4 + P.INS_TAB_H + 0.4, mode=Mode.SUBTRACT)
        # згори-ТИЛ: ніші жорстких зубів вставки, вирізані З ТИЛУ панелі
        # (24.07 ч.2, схема користувача): відкриті назад (−96.3 за тил) і
        # вниз до апертури (низ 49.9 < 50.1 → НЕ каверна); дно −98.05 =
        # стоп зуба (−97.9) від випадіння назовні (−Y), зазор 0.15
        zt0 = P.IO_Z[1] - P.INS_CLEAR                      # 49.9
        zt1 = P.IO_Z[1] + P.INS_REBATE_W - P.INS_CLEAR + 0.15   # 51.65
        yfloor_r = -96.4 - 1.5 - 0.15                      # −98.05
        for xc in P.INS_TAB_XC:
            with Locations((xc, (-96.3 + yfloor_r) / 2, (zt0 + zt1) / 2)):
                Box(P.INS_TAB_W + 0.3, -96.3 - yfloor_r, zt1 - zt0,
                    mode=Mode.SUBTRACT)
        # згори-ЛИЦЕ: ніші-ЗАГЛИБЛЕННЯ наскрізних зубців НА ЛИЦІ панелі
        # («щоб зуб був на одному рівні з фронт-панеллю»): відкриті вперед
        # (−99.5 за лице) і вниз до апертури; дно −98.1 = стоп тилу зубця
        # (−98.25) від провалювання всередину (+Y), зазор 0.15
        zh1 = P.IO_Z[1] + P.INS_HOOK_ENG + 0.15            # 51.45
        yfloor_f = -99.4 + P.INS_NICHE_D                   # −98.1
        for xh in P.INS_HOOK_XC:
            with Locations((xh, (-99.5 + yfloor_f) / 2, (zt0 + zh1) / 2)):
                Box(P.INS_HOOK_W + 0.3, yfloor_f - -99.5, zh1 - zt0,
                    mode=Mode.SUBTRACT)
        # (24.07: напливи-гачки по периметру апертури СКАСОВАНО користувачем)
        ...
        # LSI_BRK_TOP=80.75); ретенція — crush-ребра зубів у надрізах
        # обох брів (22.07: барб/кишеня лівої стінки видалені).
        ...
        with Locations(((P.ADP_X[1] + 135.2) / 2,
            ...
            Box(135.2 - P.ADP_X[1], P.CAP_SLOT_D,
                89.75 - 81.5, mode=Mode.SUBTRACT)
        # ── НАДРІЗ під ЛІВИЙ ЗУБ у ЛІВІЙ брові (22.07, дзеркально
        # правому; «защіпка провалюється при вставці зліва»): права
        # стінка різу = ліва стінка колодязя LSI (85.2); зуб t1.4 з
        # crush-ребром тисне в задню стінку −94.8. Пронг/барб/кишеня
        # барба ВИДАЛЕНІ (ретенція тепер геометрією з обох боків).
        # Друк лицем вниз: блок брови над надрізом — консоль 1.8мм
        # першого шару з L-якорем (x83.4 по всій довжині + z81.5) ──
        xwl = P.LSI_X - P.LSI_WELL_HW                     # 85.2
        with Locations(((P.CAP_NOTCH_XL + xwl) / 2,
                        (-96.4 + -96.4 + P.CAP_SLOT_D) / 2,
                        (81.5 + 89.75) / 2)):
            Box(xwl - P.CAP_NOTCH_XL, P.CAP_SLOT_D,
                89.75 - 81.5, mode=Mode.SUBTRACT)
