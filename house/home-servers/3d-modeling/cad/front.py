...
def build():
    ...
    with BuildPart() as fp:
        ...
        # знизу вгору; 22.07: бамп R0.9 з центром −96.95 — кишеня глибша
        # по Y (стінка-поріг −96.9, до тилу панелі лишається 0.5) і вища
        # по Z (52.6 — бамп-верх 52.4 + 0.2); зачеплення за стелю фальца
        # 51.7 тепер 0.7 замість 0.4)
        for xc in P.INS_DIMPLE_XC:
            with Locations((xc, (-98.05 + -96.9) / 2, 52.0)):
                Box(P.INS_LIP_TAB_W + 1.0, -96.9 - -98.05, 1.2,
                    mode=Mode.SUBTRACT)
