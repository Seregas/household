...
front.py — параметрична ФРОНТ-ПАНЕЛЬ 2U (рішення 2026-07-02):
  ...
  • права зона X92..133.4 РОЗРІЗАНА вище смуги-обідка Z0..5 (16.07) —
    знімний аддон front_addon.py; посадка: сідло = верх смуги, язики з
    crush-ребрами у раму дна, half-lap по боках; від підйому тримає
    РУКА КОВПАКА (lsi_clip.py) крізь канал у брові в нотч аддона
...
def build():
    ...
    with BuildPart() as fp:
        ...
        # ── брова у ПРОЛЬОТІ аддона: ков R5 ЛИШАЄТЬСЯ (16.07, «роби і
        # подивлюсь» — призму 15.07 прибрано, профіль брови однаковий по
        # всій хорді). Передня грань у прольоті ВТОПЛЕНА на ADP_RECESS
        # (тил аддона −96.4 не тре по брові; щілина схована за плитою
        # аддона). Різ подовжено вниз ПОВЗ низ кова: у прольоті плити
        # нема, тож хвіст профілю 0.5 (−96.9) і сліди дотику кова на
        # передній площині зрізаються цим же боксом (різ повітря ок) ──
        ax0, ax1 = P.ADP_X
        yr = -96.4 + P.ADP_RECESS
        zrec = P.PANEL_H - P.BROW_H - P.BROW_D - 0.5   # нижче низу кова
        with Locations(((ax0 + ax1) / 2, (-99.5 + yr) / 2,
                        (zrec + P.PANEL_H + 0.5) / 2)):
            Box(ax1 - ax0, yr + 99.5, P.PANEL_H + 0.5 - zrec,
                mode=Mode.SUBTRACT)

        # ── LSI (09.07 в2, «замість вбудованої»): виделки на панелі
        # НЕМА — її роль виконує ЗНІМНИЙ КОВПАК (lsi_clip.py), що
        ...
        # 16.07: колодязь розширено ВПРАВО до розрізу (перегородка 0.4
        # до аддона була тендітна — фідбек 15.07) ──
        with Locations(((P.LSI_X - P.LSI_WELL_HW + P.ADP_X[0]) / 2,
                        (-96.45 + -92.3) / 2, (78.0 + 89.75) / 2)):
            Box(P.ADP_X[0] - (P.LSI_X - P.LSI_WELL_HW),
                -92.3 - -96.45, 89.75 - 78.0, mode=Mode.SUBTRACT)
        # КАНАЛ РУКИ ковпака крізь брову у прольоті: продовження
        # колодязя вправо до нотча аддона, лише верхні Z (рука 85.6..
        # 88.75; дно каналу 85.4 → зазор 0.2); відкритий угору ──
        with Locations(((P.ADP_X[0] - 0.5 + P.ADP_NOTCH_X1 + 0.15) / 2,
                        (-96.45 + -92.3) / 2,
                        (P.ADP_NOTCH_Z0 - 0.1 + 89.75) / 2)):
            Box(P.ADP_NOTCH_X1 + 0.15 - (P.ADP_X[0] - 0.5),
                -92.3 - -96.45, 89.75 - (P.ADP_NOTCH_Z0 - 0.1),
                mode=Mode.SUBTRACT)
        # кишеня барба у ЛІВІЙ X-стінці колодязя (права стінка зникла
        # разом з перегородкою — ретенція ковпака = лівий барб + щільна
        # посадка руки в нотчі)
