...
front.py — параметрична ФРОНТ-ПАНЕЛЬ 2U (рішення 2026-07-02):
  ...
  • права зона X92..133.4 РОЗРІЗАНА між смугою-обідком Z0..5 і верхнім
    ...
    посадка: сідло = верх смуги, язики з crush-ребрами у раму дна,
    half-lap по боках; від вдавлення вгорі тримає КОВПАК-ЛЕЗО
    (addon_clip.py) у пазу передньої товщі брови, ЗА рамкою аддона
...
def build():
    ...
    with BuildPart() as fp:
        ...
        # 17.07 в3: колодязь ЗНОВУ ±LSI_WELL_HW (розширення в2 до
        # розрізу відкочено — стінка 91.6..92 більше не тендітна, бо
        # плита ободка над нею суцільна через весь проліт) ──
        with Locations((P.LSI_X, (-96.45 + -92.3) / 2, (78.0 + 89.75) / 2)):
            Box(2 * P.LSI_WELL_HW, -92.3 - -96.45, 89.75 - 78.0,
                mode=Mode.SUBTRACT)
        # ── ПАЗ КОВПАКА-ЛЕЗА у передній товщі брови (17.07 в3,
        # «защіпка-ковпачок може в товщу брови вставлятись, щоб
        # жорсткість не втратити»): вертикальний канал глибиною
        # CAP_SLOT_D від тилу плити (−96.4..−94.8), від колодязя до
        # правого краю прольоту, відкритий згори; ззаду лишається брова
        # 2.5 + ков, спереду плита ободка t3 ціла ──
        with Locations(((P.LSI_X + P.LSI_WELL_HW + P.CAP_BLADE_X1 + 0.2) / 2,
                        (-96.4 + -96.4 + P.CAP_SLOT_D) / 2,
                        (81.5 + 89.75) / 2)):
            Box(P.CAP_BLADE_X1 + 0.2 - (P.LSI_X + P.LSI_WELL_HW),
                P.CAP_SLOT_D, 89.75 - 81.5, mode=Mode.SUBTRACT)
        # МОСТООПОРИ поперек паза (17.07, друкованість лицем вниз):
        # стоять на ободку (друк h3..4.6 підпертий), перший шар брови
        # за пазом мостить між ними прольотами ≤11 замість консолі 41;
        # заразом зшивають ободок із бровою (жорсткість рами). Верх
        # опор CAP_SLOT_RIB_ZT — вище лезо аддон-кліпа несе суцільний
        # хребет (гребінка-вирізи під опори — addon_clip.py)
        for xc in P.CAP_SLOT_RIB_XC:
            with Locations((xc, -96.4 + P.CAP_SLOT_D / 2,
                            (P.ADP_RIM_Z0 + P.CAP_SLOT_RIB_ZT) / 2)):
                Box(P.CAP_SLOT_RIB_T, P.CAP_SLOT_D,
                    P.CAP_SLOT_RIB_ZT - P.ADP_RIM_Z0)
        # кишеня барба у ЛІВІЙ X-стінці колодязя (ретенція ковпака =
        # лівий барб + crush-ребра леза в задню стінку паза)
